...
class DragDropCard(QFrame):
    ...
    def set_theme(self, is_dark: bool):
        ...
        if "border: 2px solid #007acc" in self.styleSheet() or "border: 2px solid #007acc" in self._current_style:
            ...
            self._current_style = f"#DDCard {{ border: 2px dashed {self.border_normal}; border-radius: 10px; background-color: {self.bg_normal}; }}"
            self.setStyleSheet(f"{self._current_style} #DDCard:hover {{ border-color: #007acc; background-color: {self.bg_hover}; }}")
    ...
    def update_info(self, text: str):
        ...
        self._current_style = f"#DDCard {{ border: 2px solid #007acc; border-radius: 10px; background-color: {self.bg_active}; }}"
...
class TrackSelectionDialog(QDialog):
    def __init__(self, streams: list, parent=None):
        ...
        self.table.setHorizontalHeaderLabels([tr("col_faixa"), tr("col_idioma"), tr("col_codec"), tr("col_canais"), tr("col_titulo")])
...
class FFmpegInstallerDialog(QDialog):
    """Janela de verificação/instalação do FFmpeg com progresso, log e cancelamento."""
