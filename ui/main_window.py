...
def _criar_icone_check(cor: str, nome_arquivo: str, cor_borda: str = None) -> str:
    """Cria o PNG do ✓ exibido ao lado da opção marcada nos menus."""
    caminho = os.path.join(tempfile.gettempdir(), nome_arquivo)
    caminho = caminho.replace(os.sep, '/')
    if os.path.exists(caminho):
        return caminho
    ...
    if cor_borda:  # contorno p/ manter o ✓ visível no tema claro
...
CTX_MENU_CLARO_CSS = """
QMenu { background-color: #f0f0f0; border: 1px solid #cccccc; padding: 4px; }
QMenu::item { padding: 6px 28px 6px 24px; border-radius: 4px; color: #007acc; }
QMenu::item:selected { background-color: #007acc; color: #ffffff; }
QMenu::item:disabled { color: #9dc3e6; }
QMenu::separator { height: 1px; background: #cccccc; margin: 4px 8px; }
"""

CTX_MENU_ESCURO_CSS = """
QMenu { background-color: #252526; border: 1px solid #3c3c3c; padding: 4px; }
QMenu::item { padding: 6px 28px 6px 24px; border-radius: 4px; color: #ffffff; }
QMenu::item:selected { background-color: #007acc; color: #ffffff; }
...
class MainWindow(QMainWindow):
    ...
    def _setup_theme_menu(self):
        menu = QMenu(self)
        # --- Submenu Tema (ações com ✓ exclusivo) ---
        self.menu_tema = menu.addMenu(tr("menu_tema"))
        self.action_claro = QAction(tr("menu_tema_claro"), self)
        self.action_escuro = QAction(tr("menu_tema_escuro"), self)
        self.action_claro.setCheckable(True)
        self.action_escuro.setCheckable(True)
        ...
        # --- Submenu Idioma (ações com ✓ exclusivo) ---
        self.menu_idioma = menu.addMenu(tr("menu_idioma"))
        self._grupo_idioma = QActionGroup(self)
        ...
        # --- Marca inicial (✓) conforme o que está salvo ---
    ...
    def retraduzir(self):
        ...
        self.tabs.setTabText(0, tr("tab_sync"))
        self.tabs.setTabText(1, tr("tab_lipsync"))
        self.tabs.setTabText(2, tr("tab_log"))
        self.tabs.setTabText(3, tr("tab_about"))
        ...
        operacao = ("Aguardando operação...", "Waiting for operation...", "Esperando operación...")
        nova = ("Pronto para nova operação.", "Ready for a new operation.", "Listo para una nueva operación.")
        if self.progress_lbl.text() in operacao:
            self.progress_lbl.setText(tr("status_operacao"))
        elif self.progress_lbl.text() in nova:
            self.progress_lbl.setText(tr("status_nova_operacao"))
        # Reconstrói os HTMLs das abas Sobre/Lip-Sync no novo idioma
        self.atualizar_estilos_css(getattr(self.result_label, 'is_dark', True))
    ...
    def aplicar_tema_escuro(self):
        ...
        QSettings("Vicio", "DublaSync").setValue("tema", "escuro")
    ...
    def aplicar_tema_claro(self):
        ...
        QSettings("Vicio", "DublaSync").setValue("tema", "claro")
    ...
    def atualizar_estilos_css(self, is_dark: bool):
        ...
        menu_text = "#e0e0e0" if is_dark else "#333333"
        css_menu_config = f"""
            QMenu {{ background-color: {menu_bg}; color: {menu_text}; border: 1px solid {btn_border}; padding: 4px; }}
            QMenu::item {{ padding: 6px 28px 6px 8px; border-radius: 4px; color: {menu_text}; }}
            QMenu::item:selected {{ background-color: #007acc; color: #ffffff; }}
            QMenu::separator {{ height: 1px; background: {btn_border}; margin: 4px 8px; }}
        """
        # --- Indicador ✓ (somente na opção marcada), texto próximo ao ✓ ---
        sufixo_ind = "escuro" if is_dark else "claro"
        if is_dark:
            img_check = _criar_icone_check("#ffffff", f"ds_check_{sufixo_ind}.png")
        else:
            img_check = _criar_icone_check("#ffffff", f"ds_check_{sufixo_ind}.png", cor_borda="#007acc")
        css_indicadores = f"""
            QMenu::indicator {{ width: 14px; height: 14px; margin-left: 4px; }}
            QMenu::indicator:checked {{ image: url("{img_check}"); }}
        """
        css_menu_config += css_indicadores
        menu_config = self.btn_config.menu()
        menu_config.setStyleSheet(css_menu_config)
        for submenu in menu_config.findChildren(QMenu):
            submenu.setStyleSheet(css_menu_config)
        ...
        html_lipsync = (
            ...
            f"<p>{tr('lipsync_p1')}</p>"
            "<ul>"
            f"<li>{tr('lipsync_li1')}</li>"
            f"<li>{tr('lipsync_li2')}</li>"
            "</ul>"
            f"<p>{tr('lipsync_p2')}</p>"
            f"<table width='650' cellspacing='0' cellpadding='8' style='margin-top: 15px; margin-bottom: 25px; border-collapse: collapse;'>"
            f"<tr style='background-color: {table_header}; text-align: left; font-weight: bold;'>"
            f"<td width='180' style='border: 1px solid {table_border};'>{tr('col_diferenca')}</td>"
            f"<td style='border: 1px solid {table_border};'>{tr('col_percepcao')}</td>"
            "</tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🟢 0–20 ms</td><td style='border: 1px solid {table_border};'>{tr('perc_1')}</td></tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🟢 20–45 ms</td><td style='border: 1px solid {table_border};'>{tr('perc_2')}</td></tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🟡 45–90 ms</td><td style='border: 1px solid {table_border};'>{tr('perc_3')}</td></tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🟠 90–125 ms</td><td style='border: 1px solid {table_border};'>{tr('perc_4')}</td></tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🔴 125–185 ms</td><td style='border: 1px solid {table_border};'>{tr('perc_5')}</td></tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🔴 Acima de 185 ms</td><td style='border: 1px solid {table_border};'>{tr('perc_6')}</td></tr>"
            "</table>"
            f"<h3 style='color: #007acc;'>{tr('lipsync_h2')}</h3>"
            f"<p>{tr('lipsync_p3')}</p>"
            f"<table width='650' cellspacing='0' cellpadding='8' style='margin-top: 15px; border-collapse: collapse;'>"
            f"<tr style='background-color: {table_header}; text-align: left; font-weight: bold;'>"
            f"<td width='180' style='border: 1px solid {table_border};'>{tr('col_diferenca')}</td>"
            f"<td style='border: 1px solid {table_border};'>{tr('col_qualidade')}</td>"
            "</tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🟢 0–20 ms</td><td style='border: 1px solid {table_border};'>{tr('qual_1')}</td></tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🟢 20–40 ms</td><td style='border: 1px solid {table_border};'>{tr('qual_2')}</td></tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🟡 40–60 ms</td><td style='border: 1px solid {table_border};'>{tr('qual_3')}</td></tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🟠 60–100 ms</td><td style='border: 1px solid {table_border};'>{tr('qual_4')}</td></tr>"
            f"<tr><td style='border: 1px solid {table_border};'>🔴 Acima de 100 ms</td><td style='border: 1px solid {table_border};'>{tr('qual_5')}</td></tr>"
            "</table>"
            "</div>"
        )
        self.lipsync_text.setHtml(html_lipsync)
        ...
        texto_sobre = (
            ...
            f"<p style='margin-top: 0;'>{tr('sobre_p1')}</p>"
            f"<p>{tr('sobre_p2')}</p>"
            f"<p>{tr('sobre_p3')}</p>"
            f"<p>{tr('sobre_p4')}</p>"
            ...
            "<td valign='middle'>"
            f"<p style='margin: 0;'>{tr('sobre_doacao_p1')}</p>"
            f"<p style='margin-top: 10px;'>{tr('sobre_doacao_p2')}</p>"
            "</td>"
            "</tr>"
            f"<tr><td colspan='2' align='center'><img src='{qr_img}' width='170'></td></tr>"
            "<tr><td colspan='2' align='center'>"
            f"<a href='copiar_pix' style='color: #ffffff; background-color: #007acc; text-decoration: none; font-weight: bold;'>{tr('sobre_btn_pix')}</a>"
            "</td></tr>"
            f"<tr><td colspan='2' align='center'><p>{tr('sobre_obrigado')}</p></td></tr>"
            "</table>"
            "<br>"
            f"<p>{tr('sobre_dev')}<br>"
            f"{tr('sobre_canal')} <a href='https://www.youtube.com/@TutoriaisOnline/videos' style='color: #007acc; text-decoration: none; font-weight: bold;'>@TutoriaisOnline</a></p>"
            "</div>"
        )
        self.about_text.setHtml(texto_sobre)
        sufixo = "escuro" if is_dark else "claro"
        cor_seta = "#aaaaaa" if is_dark else "#666666"
        seta_cima = _criar_seta_scrollbar("up", cor_seta, f"ds_seta_cima_{sufixo}.png")
        seta_baixo = _criar_seta_scrollbar("down", cor_seta, f"ds_seta_baixo_{sufixo}.png")
        seta_esq = _criar_seta_scrollbar("left", cor_seta, f"ds_seta_esq_{sufixo}.png")
        seta_dir = _criar_seta_scrollbar("right", cor_seta, f"ds_seta_dir_{sufixo}.png")
        sb_track = "#252526" if is_dark else "#f0f0f0"
        sb_border = "#3c3c3c" if is_dark else "#cccccc"
        sb_handle = "#5a5a5a" if is_dark else "#c2c2c2"
        sb_handle_hover = "#787878" if is_dark else "#a0a0a0"
        sb_button = "#2d2d30" if is_dark else "#f0f0f0"
        scroll_css = f"""
            QScrollBar:vertical {{ background: {sb_track}; border: 1px solid {sb_border}; width: 14px; margin: 14px 0 14px 0; }}
            ...
            QScrollBar::sub-line:vertical {{ background: {sb_button}; border: 1px solid {sb_border}; height: 14px; subcontrol-position: top; subcontrol-origin: margin; }}
            QScrollBar::add-line:vertical {{ background: {sb_button}; border: 1px solid {sb_border}; height: 14px; subcontrol-position: bottom; subcontrol-origin: margin; }}
            QScrollBar::up-arrow:vertical {{ image: url("{seta_cima}"); }}
            QScrollBar::down-arrow:vertical {{ image: url("{seta_baixo}"); }}
            QScrollBar::add-page:vertical, QScrollBar::sub-page:vertical {{ background: {sb_track}; }}
            QScrollBar:horizontal {{ background: {sb_track}; border: 1px solid {sb_border}; height: 14px; margin: 0 14px 0 14px; }}
            ...
            QScrollBar::sub-line:horizontal {{ background: {sb_button}; border: 1px solid {sb_border}; width: 14px; subcontrol-position: left; subcontrol-origin: margin; }}
            QScrollBar::add-line:horizontal {{ background: {sb_button}; border: 1px solid {sb_border}; width: 14px; subcontrol-position: right; subcontrol-origin: margin; }}
            QScrollBar::left-arrow:horizontal {{ image: url("{seta_esq}"); }}
            QScrollBar::right-arrow:horizontal {{ image: url("{seta_dir}"); }}
            QScrollBar::add-page:horizontal, QScrollBar::sub-page:horizontal {{ background: {sb_track}; }}
        """
        QApplication.instance().setStyleSheet(scroll_css + css_indicadores + (CTX_MENU_ESCURO_CSS if is_dark else CTX_MENU_CLARO_CSS))
        if is_dark:
            self.progress_lbl.setStyleSheet("")
        else:
            self.progress_lbl.setStyleSheet("color: #aaaaaa;")
